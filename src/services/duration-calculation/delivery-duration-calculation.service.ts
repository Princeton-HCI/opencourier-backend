...
export class DeliveryDurationCalculationService implements IDeliveryDurationCalculationService {
  ...
  async calculateDeliveryDuration(
    ...
    durationCalculationType?: EnumDeliveryDurationCalculationType
  ): Promise<number> {
  ...
  private async resolveClass(
    durationCalculationType?: EnumDeliveryDurationCalculationType
  ): Promise<IDeliveryDurationCalculationService> {
    ...
    try {
      ...
      if (!implementation.calculateDeliveryDuration) {
        throw new Error(
          `Delivery duration calculation type ${durationCalculationType} does not implement IDeliveryDurationCalculationService`
        )
      }
      ...
      throw new Error(
        `Delivery duration calculation type not found for ${durationCalculationType}. Please check DeliveryDurationCalculationModule providers`
      )
    }
  }

  async getTypeFromDb(): Promise<EnumDeliveryDurationCalculationType> {
    const durationCalculationType =
      await this.configDomainService.instanceConfig.getDeliveryDurationCalculationTypeSetting()
